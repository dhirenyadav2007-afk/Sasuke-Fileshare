...
class MongoDB:
    ...
    def __new__(cls, uri: str, db_name: str):
        if (uri, db_name) not in cls._instances:
            ...
            instance.fbatch_groups = instance.db['fbatch_groups']  # fbatch quality-group storage
    ...
    async def save_fbatch_group(self, batch_id: str, data: dict):
        """Store a single fbatch quality-group document."""
        await self.fbatch_groups.update_one(
            {"_id": batch_id},
            {"$set": data},
            upsert=True,
        )

    async def get_fbatch_group(self, batch_id: str) -> dict | None:
        """Retrieve an fbatch group by its batch_id."""
        return await self.fbatch_groups.find_one({"_id": batch_id})

    async def delete_fbatch_group(self, batch_id: str):
        """Delete an fbatch group."""
        await self.fbatch_groups.delete_one({"_id": batch_id})

    # ✅ POST CHANNEL FUNCTIONS

    async def get_post_channels(self) -> dict:
        """Return {str(channel_id): {name, added_on}} for all post channels."""
        data = await self.user_data.find_one({"_id": "post_channels"})
        return data.get("channels", {}) if data else {}

    async def add_post_channel(self, channel_id: int, channel_data: dict):
        """Add or update a post channel entry."""
        current = await self.get_post_channels()
        current[str(channel_id)] = channel_data
        await self.user_data.update_one(
            {"_id": "post_channels"},
    ...
    async def remove_post_channel(self, channel_id: int):
        """Remove a post channel entry."""
        current = await self.get_post_channels()
        current.pop(str(channel_id), None)
        await self.user_data.update_one(
            {"_id": "post_channels"},
            {"$set": {"channels": current}},
            upsert=True,
