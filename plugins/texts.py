...
@Client.on_callback_query(filters.regex("^channels_txt$"))
async def about_txt(client: Client, query: CallbackQuery):
    await query.answer()
    ask_text = await client.ask(query.from_user.id, "Send new channels text message in the next 60 seconds, send `0` to cancel or wait 60 seconds!\n\n__Note you can use both markdown and html formatting!__", filters=filters.text, timeout=60)
    try:
        text = ask_text.text
        if text == '0':
            return await ask_text.reply("__Channels text has not changed!__")
        client.messages['ABOUT'] = text
        await texts(client, query)
        return await ask_text.reply("__Channels text has been changed!__")
    except Exception as e:
        return client.logger(__name__, client.name).error(e)
#===============================================================#
